<template>
  <el-card class="box-card" shadow="always">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <img style="height:80px;width:80px" :src="scope.row.posterurl"/>
        </template>
        </el-table-column>
      <el-table-column
        prop="type" 
        label="类别">
        <template slot-scope="scope">
          <span v-if="scope.row.type == 1">电子设备</span>
          <span v-if="scope.row.type == 2">衣服</span>
          <span v-if="scope.row.type == 3">其他</span>
          <span v-if="scope.row.type == 4">运动专区</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="score"
    ...
    <div style="padding: 14px;">
      ...
      <div class="bottom">
        <el-button type="primary" @click="navigateTo('add')">添加商品</el-button>
      </div>
