    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          ...
          <div class="pro-total pro-total-in">{{item.goodsprice*item.goodsnumber}}元</div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event,item.shopcartid,item.goodsid)"
                >确定</el-button>
              </div>
      ...
      <div class="cart-bar">
        <div class="cart-bar-left">
          ...
          <span class="cart-total">
            已选择
            <span class="cart-total-num">{{selectNum}}</span> 件
          </span>
        </div>
        <div class="cart-bar-right">
          ...
          <router-link :to="selectNum > 0 ? '/confirmOrder' : ''">
            <div :class="selectNum > 0 ? 'btn-primary' : 'btn-primary-disabled'">去结算</div>
          </router-link>
        </div>
      </div>
      <!-- 购物车底部导航条END -->
...
export default {
  ...
  methods: {
    ...
    getCartList(){
      ...
      let checkGoods = [];
      var tempmoney = 0
      for (let i = 0; i < this.getShoppingCart.length; i++) {
          const temp = this.getShoppingCart[i];
          if (temp.check) {
              checkGoods.push(temp);
              tempmoney += temp.goodsnumber*temp.goodsprice
          }
      }
        this.selectNum = checkGoods.length
        this.totalPrice = tempmoney
    },
    // 修改商品数量的时候调用该函数
    handleChange(currentValue, key, productID) {
      ...
      // 向后端发起更新购物车的数据库信息请求
      this.$axios
        .post("/api/updateCartNum", {
          uersid: this.$store.getters.getUser.id,
          goodsid: productID,
          goodsnumber: currentValue
        })
        .then(res => {
