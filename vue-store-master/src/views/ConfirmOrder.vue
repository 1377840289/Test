    ...
    <div class="content">
      ...
      <div class="section-goods">
        ...
        <div class="goods-list">
          <ul>
            <li v-for="item in getCheckGoods" :key="item.shopcartid">
              <img :src="item.imgurl" />
              <span class="pro-name">{{ item.goodsname }}</span>
              <span class="pro-price"
                >{{ item.goodsprice }}元 x {{ item.goodsnumber }}</span
              >
              <span class="pro-status"></span>
              <span class="pro-total"
                >{{ item.goodsprice * item.goodsnumber }}元</span
              >
            </li>
          </ul>
        </div>
...
export default {
  ...
  created() {
    ...
    console.log("aaa", this.getCheckGoods);
  },
  computed: {
  ...
  methods: {
    ...
    addOrder() {
      ...
      let tempdata = this.getCheckGoods;
      let arr = [];
      tempdata.forEach((element) => {
        arr.push({
          score: element.goodsprice,
          screen: element.goodsnumber,
          id: element.goodsid,
          posterurl: element.imgurl,
          wish: user.username,
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          for (let i = 0; i < products.length; i++) {
            ...
            // 删除已经结算的购物车商品
            this.deleteShoppingCart(temp.shopcartid);
          }
          // 提示结算结果
          this.notifySucceed("结算成功");
          // 跳转我的订单页面
        })
        .catch((err) => {
